...
// From Kent Boogart / http://kentb.blogspot.com/2007/04/resizer-wpf-control.html
...
namespace VizInt
{
    public enum ResizeDirection
    {
        ...
        SouthWest,
        /// <summary>
        /// Size is increased by dragging to the left.
        /// </summary>
        WestEast,
        /// <summary>
        /// Size is increased by dragging to the up.
        /// </summary>
        NorthSouth,
    }
    ...
    public class Resizer : ContentControl
    {
        ...
        {
            Size sz = SuggestedSize;
            if (double.IsNaN(Width) && !double.IsNaN(constraint.Width) && !double.IsNaN(sz.Width))
            {
                if (sz.Width < constraint.Width) constraint.Width = sz.Width;
            }
            if (double.IsNaN(Height) && !double.IsNaN(constraint.Height) && !double.IsNaN(sz.Height))
            {
                if (sz.Height < constraint.Height) constraint.Height = sz.Height;
            }
        ...
        {
            get { return (Size)GetValue(SuggestedSizeProperty); }
            set { SetValue(SuggestedSizeProperty, value); }
        }
        public static readonly DependencyProperty SuggestedSizeProperty =
            DependencyProperty.Register("SuggestedSize", typeof(Size), typeof(Resizer), new UIPropertyMetadata(new Size(double.NaN, double.NaN)));
        ...
        private static void OnUpdateSizeCommand(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            if (resizer._frameworkElement != null)
            {
                ...
                double widthDelta = double.NaN;
                double heightDelta = double.NaN;
